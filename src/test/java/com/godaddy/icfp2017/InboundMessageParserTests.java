...
public class InboundMessageParserTests {
  ...
  public void canDeserializeHandshakeS2P() throws Exception {
    ...
    final ServerToPlayer s2p = parser.getNextMessage(Resources.toString(Resources.getResource("HandshakeS2P.json"),
                                                                        Charsets.UTF_8)).get();
  ...
  public void canDeserializeSetup() throws Exception {
    InboundMessageParser parser = new InboundMessageParser();
    final ServerToPlayer s2p = parser.getNextMessage(Resources.toString(Resources.getResource("SetupS2P.json"), Charsets.UTF_8))
                                     .get();
  ...
  public void canDeserializeGameplay() throws Exception {
    InboundMessageParser parser = new InboundMessageParser();
    final ServerToPlayer s2p = parser.getNextMessage(Resources.toString(Resources.getResource("GameplayS2P.json"),
                                                                        Charsets.UTF_8)).get();
